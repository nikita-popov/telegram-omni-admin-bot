...
let getAddress = () => {
  ...
  fetch('http://ip-api.com/json', options)
    .then((res) => {
      console.log(moment().format('YYYY-MM-DD ZZ HH:mm:ss'), 'Request succes');
      return res.json();
    })
    .then((data) => {
      console.log(moment().format('YYYY-MM-DD ZZ HH:mm:ss'), `Current ip: ${data.query}`);
    })
    .catch((err) => {
      console.log(moment().format('YYYY-MM-DD ZZ HH:mm:ss'), `Fetch error: ${err.message}`);
    });
